...
class ApplicationFilters {
  ...
  def filters = {
        ...
        if (params.controller in ['recipe', 'menuPlan', 'shoppingList'] && params.action in ['show'] && params.guestVisitor) {
          ...
          if (!((params.controller in ['util', 'login', 'image', 'subscription']) ||
                  ((params.controller == 'user') &&
                          (params.action in ['create', 'createUser', 'createFreeUser', 'chooseSubscription',
                                  'newFreeUserSignUp', 'enableUser', 'newUserCheckout', 'welcome', 'facebookConnect', 'verify'])))) {
    ...
    verifyUserHasSubscription(controller: '*', action: '*') {
      before = {
        def now = new Date()

        if ((request.getSession(false) && UserTools.currentUser) &&
                (!(params.controller in ['util', 'login', 'image', 'subscription']))){
          ...
          if (!isVerified) {
            flash.message =  """The feature ${params.controller}:${params.action} you attempted to access requires a subscription and none was found.</br>
from the following active subscriptions."""

            redirect(controller: 'subscription', action: 'list', params: params)
          }
        }
      }
    }
